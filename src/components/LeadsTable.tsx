...
export default function LeadsTable({ leads, locale = "en" }: { leads: Lead[]; locale?: Locale }) {
  const t = dictionaries[locale].leadsPage;
  ...
  async function routeToMarketing() {
    ...
    try {
      ...
      setNotice(tf(t.routedNotice, { count: data.updated }));
      setSelected(new Set());
      router.refresh();
    } catch {
      setNotice(t.routeError);
    } finally {
      setRouting(false);
    }
  }
          ...
          {selected.size > 0 ? (
            <>
              <span className="text-sm font-medium text-slate-700 dark:text-slate-200">
                {tf(t.selected, { count: selected.size })}
              </span>
              <a
              ...
              >
                {routing ? t.routing : t.routeToMarketing}
              </button>
              <button
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500">{t.selectHint}</p>
          )}
        </div>
        <a href={exportUrl()} className="text-sm font-medium text-amber-700 hover:text-amber-800 dark:text-amber-400 dark:hover:text-amber-300">
          {tf(t.exportAll, { count: leads.length })}
        </a>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">
                ...
                <th className="px-4 py-2.5 font-medium">{t.colOwner}</th>
                <th className="px-4 py-2.5 font-medium">{t.colCondition}</th>
                <th className="px-4 py-2.5 font-medium">{t.colRoof}</th>
                <th className="px-4 py-2.5 font-medium">{t.colAge}</th>
                <th className="px-4 py-2.5 font-medium">{t.colQuote}</th>
